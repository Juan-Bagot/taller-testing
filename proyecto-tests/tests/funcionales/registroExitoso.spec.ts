...
test("TC01 — Registro exitoso de paciente", async ({ page }) => {
  ...
  const mensajeFlash = page.getByText(
    "Cuenta creada. Ya podés iniciar sesión.",
  );
  await expect(mensajeFlash).toBeVisible();

  // Resultado esperado 3: Login con nuevas credenciales
  await page.getByLabel("Email", { exact: true }).fill(email);
  await page.getByLabel("Contraseña", { exact: true }).fill(password);
  await page.getByRole("button", { name: "Entrar" }).click();

  const flash = page.getByText(`Hola, ${nombre}`);
  await expect(flash).toBeVisible();
  // Confirmar que el nombre del usuario aparece exacto en la barra de navegación
